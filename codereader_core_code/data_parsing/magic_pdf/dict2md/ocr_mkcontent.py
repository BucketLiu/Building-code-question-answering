...
from data_parsing.magic_pdf.config.make_content_config import DropMode, MakeMode
from data_parsing.magic_pdf.config.ocr_content_type import BlockType, ContentType
from data_parsing.magic_pdf.libs.commons import join_path
from data_parsing.magic_pdf.libs.language import detect_lang
from data_parsing.magic_pdf.libs.markdown_utils import ocr_escape_special_markdown_char
from data_parsing.magic_pdf.para.para_split_v3 import ListLineTag
...
def union_make(pdf_info_dict: list,
    ...
    for page_info in pdf_info_dict:
        ...
        if make_mode == MakeMode.MM_MD:
            page_markdown = ocr_mk_markdown_with_para_core_v2(
                paras_of_layout, 'mm', img_buket_path)
            page_markdown.append("{[hhh]}")  # 添加分页符
            ...
            output_content.append("{[hhh]}")  # 添加分页符

    if make_mode in [MakeMode.MM_MD, MakeMode.NLP_MD]:
        return '\n\n'.join(output_content)  # 返回 Markdown 内容时，分页符在各页内容之间
    elif make_mode == MakeMode.STANDARD_FORMAT:
        return output_content  # 返回标准格式内容时，分页符在各页内容之间
