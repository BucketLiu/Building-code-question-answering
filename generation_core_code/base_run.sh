...
update_or_append_to_env() {
  local key=$1
  local value=$2
  local env_file="kernel/configs/model_config.py"
...
# 确保必需参数已提供
if [ -z "$system" ] || [ -z "$milvus_port" ] || [ -z "$port" ]; then
    echo "必须提供 --system, --milvus_port 和 --port 参数。"
    exit 1
fi
