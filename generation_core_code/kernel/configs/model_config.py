...
PDF_MODEL_PATH = os.path.join(root_path, "kernel/utils/loader/pdf_to_markdown")

nltk_data_path = os.path.join(root_path, 'kernel/nltk_data')

# 将你的本地路径添加到nltk的数据路径中
nltk.data.path.append(nltk_data_path)

OCR_MODEL_PATH = os.path.join(root_path, "kernel", "dependent_server", "ocr_server", "ocr_models")
...
{question}
---
你是一个专业的工程规范助手，能够准确回答与工程规范相关的问题。请根据以下规则完成任务：题目会提出一个问题，你需要根据工程规范原文内容，直接回答问题，不要添加个人观点，不要返回题目原文，返回的答案不要带有任何文本格式，确保答案简洁。
"""
...
SQLITE_DATABASE = os.path.join(root_path, "QANY_DB", "thing.db")
...
if os_system == 'Darwin':
    DT_3B_MODEL_PATH = os.path.join(model_path, "MiniChat-2-3B-FP16-GGUF") + '/MiniChat-2-3B-fp16.gguf'
    DT_3B_DOWNLOAD_PARAMS = {'model_id': 'MiniChat-2-3B-FP16-GGUF',
                             'revision': 'master', 'cache_dir': model_path}
    DT_CONV_3B_TEMPLATE = "minichat"
    DT_7B_MODEL_PATH = ''
    DT_7B_DOWNLOAD_PARAMS = {}
    DT_CONV_7B_TEMPLATE = "buildingqa-7b"
else:
    DT_3B_MODEL_PATH = os.path.join(model_path, "MiniChat-2-3B")
    DT_7B_MODEL_PATH = os.path.join(model_path, "buildingqa-7b")
    DT_3B_DOWNLOAD_PARAMS = {'model_id': 'MiniChat-2-3B',
                             'revision': 'master', 'cache_dir': model_path}
    DT_7B_DOWNLOAD_PARAMS = {'model_id': 'buildingqa-7b',
                             'revision': 'master', 'cache_dir': model_path}
    DT_CONV_3B_TEMPLATE = "minichat"
    DT_CONV_7B_TEMPLATE = "buildingqa-7b"