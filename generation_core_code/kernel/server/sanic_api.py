...
# 获取当前脚本的父目录的路径，即`server`目录
current_dir = os.path.dirname(current_script_path)

# 获取`server`目录的父目录，即`kernel`
...
from kernel.configs.model_config import DT_7B_MODEL_PATH, \
    DT_7B_DOWNLOAD_PARAMS, DT_3B_MODEL_PATH, DT_3B_DOWNLOAD_PARAMS, PDF_MODEL_PATH
import kernel.configs.model_config as model_config
from kernel.utils.custom_log import debug_logger
from kernel.utils.general_utils import download_file, get_gpu_memory_utilization, check_package_version
import torch
import platform
...
parser.add_argument('--host', dest='host', default='0.0.0.0', help='set host for thing server')
parser.add_argument('--port', dest='port', default=8777, type=int, help='set port for thing server')
...
#  必填参数
parser.add_argument('--model_size', dest='model_size', default='7B', help='set LLM model size for thing server')
parser.add_argument('--device_id', dest='device_id', default='0', help='cuda device id for thing server')
...
from .handler import *
from kernel.core.local_doc_qa import LocalDocQA
...
# 将 /thing 路径映射到 ./dist/thing 文件夹，并指定路由名称
app.static('/thing/', 'kernel/server/dist/thing/', name='thing', index="index.html")
...
async def print_info(app, loop):
    print(f"已启动后端服务，请复制[  http://0.0.0.0:{args.port}/thing/  ]到浏览器进行测试。", flush=True)
...
async def start_server_and_open_browser(app, loop):
    try:
        print(f"Opening browser at http://{args.host}:{args.port}/thing/")
        webbrowser.open(f"http://{args.host}:{args.port}/thing/")
