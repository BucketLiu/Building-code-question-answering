from kernel.core.local_file import LocalFile
...
from kernel.configs.model_config import BOT_DESC, BOT_IMAGE, BOT_PROMPT, BOT_WELCOME
...
async def document(req: request):
    description = """
# thing 介绍
[戳我看视频>>>>>【有道thing介绍视频.mp4】](https://docs.popo.netease.com/docs/7e512e48fcb645adadddcf3107c97e7c)

**thing** (**Q**uestion and **A**nswer based on **Anything**) 是支持任意格式的本地知识库问答系统。
...
async def new_bot(req: request):
    ...
    local_doc_qa.mysql_client.new_thing_bot(bot_id, user_id, bot_name, desc, head_image, prompt_setting,
                                                welcome_message, model, kb_ids_str)
    create_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return sanic_json({"code": 200, "msg": "success create thing bot {}".format(bot_id),
...
async def get_qa_info(req: request):
    ...
    if save_to_excel:
        ...
        file_name = f"thing_QA_{timestamp}.xlsx"
