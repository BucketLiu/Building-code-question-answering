import sqlite3
from kernel.configs.model_config import SQLITE_DATABASE 
from kernel.utils.custom_log import debug_logger
import uuid
import json
...
class KnowledgeBaseManager:
    ...
    def create_tables_(self):
        ...
        query = """
            CREATE TABLE IF NOT EXISTS thingBot (
    ...
    def check_bot_is_exist(self, bot_id):
        # 使用参数化查询
        query = "SELECT bot_id FROM thingBot WHERE bot_id = ? AND deleted = 0"
    ...
    def new_thing_bot(self, bot_id, user_id, bot_name, description, head_image, prompt_setting, welcome_message, model, kb_ids_str):
        query = "INSERT INTO thingBot (bot_id, user_id, bot_name, description, head_image, prompt_setting, welcome_message, model, kb_ids_str) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    def delete_bot(self, user_id, bot_id):
        # 使用参数化查询
        query = "UPDATE thingBot SET deleted = 1 WHERE user_id = ? AND bot_id = ?"
    ...
    def get_bot(self, user_id, bot_id):
        if not bot_id:
            query = "SELECT bot_id, bot_name, description, head_image, prompt_setting, welcome_message, model, kb_ids_str, update_time, user_id FROM thingBot WHERE user_id = ? AND deleted = 0"
            return self.execute_query_(query, (user_id,), fetch=True)
        elif not user_id:
            query = "SELECT bot_id, bot_name, description, head_image, prompt_setting, welcome_message, model, kb_ids_str, update_time, user_id FROM thingBot WHERE bot_id = ? AND deleted = 0"
            return self.execute_query_(query, (bot_id, ), fetch=True)
        else:
            query = "SELECT bot_id, bot_name, description, head_image, prompt_setting, welcome_message, model, kb_ids_str, update_time, user_id FROM thingBot WHERE user_id = ? AND bot_id = ? AND deleted = 0"
    ...
    def update_bot(self, user_id, bot_id, bot_name, description, head_image, prompt_setting, welcome_message, model,
        ...
        query = "UPDATE thingBot SET bot_name = ?, description = ?, head_image = ?, prompt_setting = ?, welcome_message = ?, model = ?, kb_ids_str = ?, update_time = ? WHERE user_id = ? AND bot_id = ? AND deleted = 0"
